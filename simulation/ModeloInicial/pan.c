...
short src_ln7 [] = {
	  0, 183, 187, 188, 189, 190, 191, 186, 
	195, 186, 196, 197, 198, 199, 200, 201, 
	202, 197, 206, 197, 206, 207, 208, 209, 
	212, 214,   0, };
S_F_MAP src_file7 [] = {
	{ "-", 0, 0 },
	{ "trabajo.pml", 1, 25 },
...
short src_ln1 [] = {
	  0,  70,  72,  73,  74,  78,  79,  84, 
	 85,  86,  87,  90,  75,  71,  93,  69, 
	 95,  69,  95,  96,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "trabajo.pml", 1, 19 },
...
short src_ln0 [] = {
	  0,  40,  43,  44,  45,  46,  39,  51, 
	 39,  51,  53,  54,  55,  56,  57,  52, 
	 60,  52,  60,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "trabajo.pml", 1, 18 },
...
{	int j = 0, h = now._nr_pr;
	...
	case 7:	/* :init: */
		...
		((P7 *)pptr(h))->k = 0;
#ifdef VAR_RANGES
		logval(":init::i", ((P7 *)pptr(h))->i);
		logval(":init::j", ((P7 *)pptr(h))->j);
		logval(":init::k", ((P7 *)pptr(h))->k);
#endif
#ifdef HAS_CODE
		locinit7(h);
#endif
	...
	case 6:	/* Aggregator */
		...
		((P6 *)pptr(h))->ty = 0;
#ifdef VAR_RANGES
		logval("Aggregator:us", ((P6 *)pptr(h))->us);
		logval("Aggregator:ta", ((P6 *)pptr(h))->ta);
		logval("Aggregator:pa", ((P6 *)pptr(h))->pa);
		logval("Aggregator:ty", ((P6 *)pptr(h))->ty);
#endif
#ifdef HAS_CODE
		locinit6(h);
#endif
	...
	case 5:	/* DetectTypes */
		...
		/* params: */
		/* locals: */
		((P5 *)pptr(h))->anomalous = 0;
#ifdef VAR_RANGES
		logval("DetectTypes:anomalous", ((P5 *)pptr(h))->anomalous);
#endif
#ifdef HAS_CODE
		locinit5(h);
#endif
	...
	case 4:	/* DetectPairs */
		...
		/* params: */
		/* locals: */
		((P4 *)pptr(h))->anomalous = 0;
#ifdef VAR_RANGES
		logval("DetectPairs:anomalous", ((P4 *)pptr(h))->anomalous);
#endif
#ifdef HAS_CODE
		locinit4(h);
#endif
	...
	case 3:	/* DetectTargets */
		...
		/* params: */
		/* locals: */
		((P3 *)pptr(h))->anomalous = 0;
#ifdef VAR_RANGES
		logval("DetectTargets:anomalous", ((P3 *)pptr(h))->anomalous);
#endif
#ifdef HAS_CODE
		locinit3(h);
#endif
	...
	case 2:	/* DetectUsers */
		...
		/* params: */
		/* locals: */
		((P2 *)pptr(h))->anomalous = 0;
#ifdef VAR_RANGES
		logval("DetectUsers:anomalous", ((P2 *)pptr(h))->anomalous);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
...
{	int j=0, i = now._nr_qs;
	...
	case 1: j = sizeof(Q1); q_flds[1] = 3; q_max[1] = 50; break;
...
{	/* int i; */
	...
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	int    UserConts[%d]:	%d\n", l_in, now.UserConts[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	int    TargetConts[%d]:	%d\n", l_in, now.TargetConts[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	int    PairConts[%d]:	%d\n", l_in, now.PairConts[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	int    TypeConts[%d]:	%d\n", l_in, now.TypeConts[l_in]);
		}
	}
}
void
...
{	/* int i; */
	...
	printf("	int    anomalous:	%d\n", ((P5 *)pptr(pid))->anomalous);
		break;
	case 4:
		printf("local vars proc %d (DetectPairs):\n", pid);
	printf("	int    anomalous:	%d\n", ((P4 *)pptr(pid))->anomalous);
		break;
	case 3:
		printf("local vars proc %d (DetectTargets):\n", pid);
	printf("	int    anomalous:	%d\n", ((P3 *)pptr(pid))->anomalous);
		break;
	case 2:
		printf("local vars proc %d (DetectUsers):\n", pid);
	printf("	int    anomalous:	%d\n", ((P2 *)pptr(pid))->anomalous);
		break;
	case 1:
		printf("local vars proc %d (Worker):\n", pid);
	printf("	byte   id:	%d\n", ((P1 *)pptr(pid))->id);
	printf("	int    userKey:	%d\n", ((P1 *)pptr(pid))->userKey);
	printf("	int    targetKey:	%d\n", ((P1 *)pptr(pid))->targetKey);
	printf("	int    typeKey:	%d\n", ((P1 *)pptr(pid))->typeKey);
		break;
	case 0:
